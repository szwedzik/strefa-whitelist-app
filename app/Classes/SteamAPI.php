...
namespace App\Classes;
	...
	class SteamAPI {
		...
		public function GetPlayerSteamID($vanity) {
			$url = $this->pre_url . 'ISteamUser/ResolveVanityURL/v1/?key=' . $this->api_key . '&vanityurl=' . $vanity;
			$contents = $this->get_content($url);
			$contents = $contents['response'];

			$steamid = isset($contents['steamid']) ? $contents['steamid'] : "none";
